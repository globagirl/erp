...
<tr>
  ...
  <th style="width:10%;height:60px" class="degraD2"><b>OF</b></th>
  ...
  <th style="width:7%;height:60px" class="degraD2"><b>Gross margin %</b></th>
    ...
    if($recherche == "A"){
		...
		$reqSup1= mysql_query("SELECT OF,PO,produit,qte,date_exped_conf,statut FROM ordre_fabrication1 WHERE  (date_exped_conf >= '$date1') and (date_exped_conf <= '$date2')");
		$reqSup2= mysql_query("SELECT PO,produit,qty,dateExp,statut,prixU FROM commande_items WHERE  (dateExp >= '$date1') and (dateExp <= '$date2') and ($recherche='$valeur') and ((statut LIKE '%waiting%') or (statut LIKE '%incomplete%'))");
	}else{
		...
		$reqSup1= mysql_query("SELECT OF,PO,produit,qte,date_exped_conf,statut FROM ordre_fabrication1 WHERE  (date_exped_conf >= '$date1') and (date_exped_conf <= '$date2') and $recherche='$valeur'");
		$reqSup2= mysql_query("SELECT PO,produit,qty,dateExp,statut,prixU FROM commande_items WHERE  (dateExp >= '$date1') and (dateExp <= '$date2') and ($recherche='$valeur') and ((statut LIKE '%waiting%') or (statut LIKE '%incomplete%'))");
	}
//Traitement partie 1 // commande déja planifié	
//Sortie du magazin 
    $i=1;
    while($a=@mysql_fetch_object($reqSup)){
		...
		$statOF=$a->statut;
		//prix unitaire
		$req=@mysql_query("SELECT prixU FROM commande_items where POitem='$PO'");
		$prixU=@mysql_result($req,0);
		$prixT=$prixU*$qteC;
		//theoritical cost
		...
		while($data1=@mysql_fetch_object($req1)){
		    ...
		    $totalTC=$totalTC+$prixTItem;	//theoritical cost		
	    }	
            //Cost	
		if($statOF != 'planned'){
			$totalItem=0;
	    ...
	    $profit=$prixT-$totalItem; //profit	   
	    //Affichage
		$P1=$totalTC/$prixT;
		$P2=(1-$P1)*100;
		$P2=round($P2,2);
		
		$W1=$totalItem-$totalTC;//waist
		$W1=round($W1,2);
		$W2=$W1/$totalTC;
		$W2=$W2*100;
		$W2=round($W2,2); // waist %
