...
for($i=0; $i<count($fichier['name']); $i++) {
    ...
    if(isset($_POST['paid1'])){//si la facture est déja payé
        ...
        $sql2="INSERT INTO supplier_invoice(IDinvoice, supplier, dateE,dateF,dateP,total,currency,coursTND, status,typeI,catI) VALUES ('$IDinvoice','$supplier',NOW(),'$dateF','$dateP','$total','$devise','$coursTND','paid','$typeI','$catI')";
        if (!mysql_query($sql2)) {
        ...
        $sql2="INSERT INTO supplier_invoice(IDinvoice, supplier, dateE,dateF,total,currency,coursTND, status,typeI,catI) VALUES 
            ('$IDinvoice','$supplier',NOW(),'$dateF','$total','$devise','$coursTND','unpaid','$typeI','$catI')";
        if (!mysql_query($sql2)) {
            die('Error: ' . mysql_error());
            header('Location: ../pages/supplier_invoice.php?status=fail');
        }
