...
function afficheLigneTheo($col1,$long,$OF,$PO,$dateE,$produit,$qty,$revenue,$theo_coast,$theo_coast2,$gross,$grossP,$averageC){
    if($theo_coast< $revenue){
        ...
        echo"<tr >
        <td  style=\"width:15%;height:60px;background-color:".$col1."\">$PO</td>
        <td  style=\"width:10%;height:60px;background-color:".$col1."\">$dateE</td>
        <td  style=\"width:15%;height:60px ;background-color:".$col1."\">$produit</td>
        <td  style=\"width:6%;height:60px ;background-color:".$col1."\">$long M</td>
        <td  style=\"width:10%;height:60px ;background-color:".$col1."\">$qty</td>
        <td  style=\"width:8%;height:60px;background-color:".$col."\">$revenue</td>
        <td  style=\"width:15%;height:60px;background-color:".$col."\"><b>$theo_coast</b></td>
     
        <td  style=\"width:8%;height:60px;background-color:".$col1."\">$gross</td>
        <td  style=\"width:8%;height:60px ;background-color:".$col1."\">$grossP %</td>
        <td  style=\"width:5%;height:60px;background-color:".$col1."\">$averageC</td>	";
...
<tr>
  ...
  <th style="width:5.8%;height:60px" class="degraD2"><b>Lenght</b></th>
...
while($data=mysql_fetch_array($sql1)){
   ...
   $reqCat= mysql_query("SELECT categorie FROM produit1 where code_produit='$produit'");
   $catRes=mysql_result($reqCat,0);
   if($cat=="A"){
      $cond=true;
   }else if($cat=="C"){
    if($catRes != "QUICK FIT"){
...
//Les totals
 if($i>0){
   $total_gross_marginP=$total_gross_margin/$total_revenue;
 }
 ...
 $total_average=round($total_average,2);
  ...
  <tr>
  <td  style=width:30%;height:40px;background-color:#F8E0EC;text-align:right ><b>Total coast: </b></td>
  ...
  <tr><td style=width:30%;height:40px;text-align:right;background-color:#FAB6B6 ><b>Gross margin %: </b></td>
  <td style=width:70%;height:40px;background-color:#FAB6B6><b> '.$total_gross_marginP.' %</b></td></tr>
  <tr>
