        ...
        <script>
            ...
            function affiche_reception(){
                ...
                if ((VF==1) || (typeI=="M")){
                    ...
                    var invoice = document.getElementById("invoice").value;
                    if(valeur=="s"){
                        alert("PLZ select Supplier !!");
                    }else if(invoice=="" && typeI != "M"){
                        alert("PLZ verify invoice N° !!");
                ...
                }else{alert("Check your invoice N° PLZ !!"); }
            }
            /////
            function pop_up(url){
                window.open(url,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,' +
                    'width=1076,height=768,directories=no,location=no')
            }
            ////
            ...
            function affichelisteF(){
                ...
                //
                if(typeI=="Expense"){
                    $('#TDsupplier').html('<input type="text" id="supplier" name="supplier" onKeyup="autoComplete();" ' +
                        'onFocus="autoComplete()" onBlur="hideListe();">  <div class="divAuto2"><ul id="listeF2" ></ul></div>');
                }else{
                    $('#TDsupplier').html('	<span class="custom-dropdown custom-dropdown--white custom-dropdown--small">' +
                        '<select name="supplier" id="supplier"  style="width:220px" class="custom-dropdown__select ' +
                        'custom-dropdown__select--white" onChange="clearZone();" onFocus="affichelisteF2();">' +
                        '<option value="s">---Select---</option> </select> </span> ' +
                        '<input type="button" onclick=pop_up("../pages/pop_ajout_fournisseur.php") value="+"> ');
                }
            }
            ...
            function addI1(){
                if(VF==1){
                    ...
                    if(coursTND != ""){
                        ...
                        alert("PLZ enter the TND Course !!");
                    }
                }else{
                    alert("Check your Invoice N° PLZ !!");
                }
            }
            //Credit note
            function addI4(){
...
//alert(invoice);
                ...
                if(verif==0){
                    ...
                    alert("Invoice already exist, PLZ Verify !");
                }
            }
            ...
            function addZ(){
                ...
                $('#divAdd').append('<table id='+Z+'><tr><td><center><b> Invoice N°: </b>' +
                    '<input type="text" placeholder="Invoice N°" name='+inv+' id='+inv+' onBlur=verif_invoice2("'+inv+'")>' +
                    '</center></td><td><b>Date facturation : </b><input  type="date" name='+dateF+' ></td><td><b>Amount : ' +
                    '</b><input  type="text" name='+total+'  size=\"15 \" >' +
                    '<span class="custom-dropdown custom-dropdown--white custom-dropdown--small"> ' +
                    '<select name='+D+' type="text" class="custom-dropdown__select custom-dropdown__select--white">' +
                    '<option value="TND">TND</option><option value="Euro">Euro</option><option value="Dollar">Dollar</option>' +
                    '</select></span> <input  type="text" id='+coursTND+' name='+coursTND+'  placeholder="Cours TND" size="10" >' +
                    '</td></tr><tr><td><center> <b>Paid  </b> ' +
                    '<input type="checkbox" name='+p+' id='+p+' value="oui" onClick=activeMP("'+x+'")></center></td><td>' +
                    '<b>Date Payement : </b><input type="date" name='+dateP+' id='+dateP+'></td> <td>' +
                    '<span class="custom-dropdown custom-dropdown--white custom-dropdown--small" >' +
                    '<select id='+modeP+' name='+modeP+' onChange=afficheMP("'+x+'") class="custom-dropdown__select ' +
                    'custom-dropdown__select--white"  disabled>' +
                    '<option value="Cache">Cache</option> <option value="Cheque">Par chéque</option>   ' +
                    '<option value="Virement">Virement</option><option value="Autre">Autre..</option>' +
                    '</select></span></td></tr><tr><td id='+zoneMP+' colspan="2" style="text-align:right">' +
                    '</td><td><center><input type="file" name='+img+'  multiple></center></td></tr></table>');
            ...
            function afficheMP(i){
                ...
                if(val == "Autre"){
                    $(zoneMP).html('<input type="text"  id='+ref+' name='+ref+' size="40" placeholder="Payment Mode" > ');
                }else if(val != "Cache"){
                    $(zoneMP).html('<input type="text"  id='+ref+' name='+ref+' size="25" placeholder="Reference" >' +
    ...
    <div id="main">
        ...
        <div id='contenu'>
            ...
            <form method="POST"  id="form1" name="form1" action="../php/supplier_invoice2.php" enctype="multipart/form-data">
                <TABLE >
                    <TR>
                        ...
                        <TD >
			<span class="custom-dropdown custom-dropdown--white custom-dropdown--small">
            <select name="typeI" id="typeI" onChange="affichelisteF(),affichelisteF2(),clearZone();"
                    style="width:150px" class="custom-dropdown__select custom-dropdown__select--white" >
             ...
             <option value="Purchase">Purchase</option>
			  ...
			  <option value="Expense">Expense</option>
			   ...
			   <option value="M">Multiple service</option>
                        ...
                        <TD id="TDsupplier">
			<span class="custom-dropdown custom-dropdown--white custom-dropdown--small">
            <select name="supplier" id="supplier"  style="width:220px"
                    class="custom-dropdown__select custom-dropdown__select--white" onChange="clearZone();">
			...
			<span class="custom-dropdown custom-dropdown--white custom-dropdown--small">
            <select name="catI" id="catI"  style="width:200px" class="custom-dropdown__select custom-dropdown__select--white"
                    onFocus="afficheCat();">
            </select>
            </span>
                        </td>
                        ...
                        <td><input type="text" name="invoice" size="15" id="invoice" placeholder="Invoice N°"
                                   onBlur="verif_invoice();"></td>
...
if(!empty($_GET['status']))
{
    $status = $_GET['status'];
    if  ($status=="sent")
    {echo '<SCRIPT LANGUAGE="JavaScript">alert(\'Invoice added successfully \');</SCRIPT>';
