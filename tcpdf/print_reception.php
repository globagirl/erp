...
$pdf->MultiCell(90, 8, 'Date reception : '.$dateR, 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(95, 8, 'Fournisseur : '.$fournisseur, 1, 'C', 1, 0, '', '', true);
$pdf->ln(11);
$sql1 =mysql_query("SELECT item AS item,sum(qty) AS somP ,IDorder AS IDorder FROM reception_items where IDreception IN (SELECT IDreception FROM reception where dateR='$dateR' and supplier='$fournisseur' and (status='received' or status='invoiced')) group by item,IDorder");
$pdf->SetFont('times', 'B', 12);
$pdf->MultiCell(15, 7, 'N°', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(50, 7, 'ID Ordre', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(60, 7, 'Article', 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(60, 7, 'Quantité', 1, 'C', 1, 0, '', '', true);
 $pdf->ln(7);
 $pdf->SetFont('times',  11);
$i=0;
while ($data1=mysql_fetch_array($sql1)){
$i++;
$pdf->MultiCell(15, 6, $i, 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(50, 6, $data1['IDorder'], 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(60, 6, $data1['item'], 1, 'C', 1, 0, '', '', true);
$pdf->MultiCell(60, 6, $data1['somP'], 1, 'C', 1, 0, '', '', true);
