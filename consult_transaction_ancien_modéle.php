 <?php
    ...
    $compte=$_POST['compte'];		    
    $req1= "SELECT * FROM transaction_compte WHERE  compte='$compte'  and etat != 'AT' order by IDtrans ASC ";
    $req2= "SELECT * FROM transaction_compte WHERE  compte='$compte' etat = 'AT' order by IDtrans ASC ";
    ...
    while($a=mysql_fetch_array($r1)){ 
        ...
        */		
		...
		<td  style="width:11%;height:65px;text-align:center"><font color='.$col.'>'.$a['dateT'].'</font></td>
		<td  style="width:7%;height:65px;text-align:center"><font color='.$col.'><b>'.$a['modeT'].'</b></font></td>
		<td style="width:12%;height:65px;text-align:center"><font color='.$col.'>'.$a['ref'].'</font></td>
		<td style="width:24%;height:65px;text-align:center"><font color='.$col.'>'.$a['catT'].' : '.$a['description'].'</font></td>');
        if($typeT=='RT'){		
