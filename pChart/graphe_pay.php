...
while($a=@mysql_fetch_object($req)){
	...
	while($data=mysql_fetch_object($req1)){
      ...
      $prixT=$prix+$prixT;
	    //theoretical cost
	    //$totalTC=calcul_theo_coast($produit,$qteC,$dateE);
      //Real coast
      $totalTC=calcul_real_coast($OF);
  }
  $payT=$prixT-$totalTC;
}
  $payT=round($payT,2);
...
$myPicture->drawThreshold(28000,array("WriteCaption"=>TRUE,"Caption"=>"Limit"));
...
$randX=rand().rand(1,9);
 $myPicture->Render("img/PAY".$randX.".png");
  echo('<img src="../pChart/img/PAY'.$randX.'.png"  alt="Print" style="cursor:pointer;" />');
?>
